"use client";

import React from "react";
import { useTheme } from "next-themes";
import * as echarts from "echarts/core";
import { BarChart } from "echarts/charts";
import {
  TitleComponent,
  TooltipComponent,
  GridComponent,
  LegendComponent,
} from "echarts/components";
import { CanvasRenderer } from "echarts/renderers";
import ReactECharts from "echarts-for-react";
import styles from "./ApacheMarimekkoChart.module.scss";
import { ApacheMarimekkoChartProps } from "../common/types";

echarts.use([
  TitleComponent,
  TooltipComponent,
  GridComponent,
  LegendComponent,
  BarChart,
  CanvasRenderer,
]);

const ApacheMarimekkoChart: React.FC<ApacheMarimekkoChartProps> = ({
  title = "Marimekko Chart",
  data,
  xAxisLabel = "Category",
  yAxisLabel = "Percentage",
}) => {
  const { theme } = useTheme();
  const isDark = theme === "dark";

  const categories = data.map((item) => item.x);
  const seriesNames = Object.keys(data[0]).filter((key) => key !== "x");

  const seriesData = seriesNames.map((name) => ({
    name,
    type: "bar",
    stack: "total",
    emphasis: { focus: "series" },
    barWidth: (data.map((item) => item.width) as number[]),
    data: data.map((item) => item[name] as number),
  }));

  const options = {
    backgroundColor: isDark ? "#1A1A2E" : "#FFFFFF",
    title: {
      text: title,
      left: "center",
      textStyle: { color: isDark ? "#FFFFFF" : "#333" },
    },
    tooltip: {
      trigger: "axis",
      axisPointer: { type: "shadow" },
      formatter: (params: any) => {
        let tooltipContent = `<b>${params[0].axisValue}</b><br/>`;
        params.forEach((item: any) => {
          tooltipContent += `<span style="display:inline-block;width:10px;height:10px;margin-right:5px;background:${item.color};"></span>
            ${item.seriesName}: <b>${item.value}%</b><br/>`;
        });
        return tooltipContent;
      },
    },
    legend: { top: 30, textStyle: { color: isDark ? "#FFFFFF" : "#333" } },
    grid: { left: "5%", right: "5%", bottom: "10%", containLabel: true },
    xAxis: {
        type: "category",
        name: xAxisLabel, // Make it reusable
        nameLocation: "center", // ✅ Centering the label
        nameTextStyle: {
          fontSize: 14,
          padding: [10, 0, 0, 0], // Adjust padding if needed
          color: isDark ? "#FFFFFF" : "#333",
        },
        data: categories,
        axisLine: { lineStyle: { color: isDark ? "#FFFFFF" : "#333" } },
      },
      yAxis: {
        type: "value",
        name: yAxisLabel, // Make it reusable
        nameLocation: "center", // ✅ Centering the label
        nameTextStyle: {
          fontSize: 14,
          padding: [0, 0, 10, 0], // Adjust padding if needed
          color: isDark ? "#FFFFFF" : "#333",
        },
        axisLine: { lineStyle: { color: isDark ? "#FFFFFF" : "#333" } },
      },
    series: seriesData,
  };

  return (
    <div className={styles.chartContainer}>
      <ReactECharts option={options} style={{ height: "500px", width: "100%" }} />
    </div>
  );
};

export default ApacheMarimekkoChart;
